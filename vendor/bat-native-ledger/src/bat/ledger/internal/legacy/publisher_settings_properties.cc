...
bool PublisherSettingsProperties::FromValue(const base::Value::Dict& dict) {
  ...
  // Allow Non Verified
  if (auto allow_non_verified_sites_in_list_value =
          dict.FindBool(kAllowNonVerifiedSitesInListKey)) {
    allow_non_verified_sites_in_list = *allow_non_verified_sites_in_list_value;
  } else {
    NOTREACHED();
    return false;
  }
  ...
  // Allow Videos
  if (auto allow_contribution_to_videos_value =
          dict.FindBool(kAllowContributionToVideosKey)) {
    allow_contribution_to_videos = *allow_contribution_to_videos_value;
  } else {
    NOTREACHED();
    return false;
  }
