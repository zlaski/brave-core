...
EthereumRemoteCilentService::EthereumRemoteCilentService(
  ...
  pref_change_registrar_->Add(
      ...
      base::Bind(&EthereumRemoteCilentService::OnPreferenceChanged,
...
const size_t EthereumRemoteCilentService::kNonceByteLength = 12;
const size_t EthereumRemoteCilentService::kSeedByteLength = 32;
...
// static
std::string EthereumRemoteCilentService::GetBitGoSeedFromRootSeed(
...
// https://github.com/brave/brave-browser/wiki/Brave-Ethereum-Remote-Client-Wallet-Seed-Information#note-on-salts
// static
std::string EthereumRemoteCilentService::GetEthereumRemoteClientSeedFromRootSeed(
    const std::string& seed) {
...
// static
bool EthereumRemoteCilentService::LoadFromPrefs(PrefService* prefs,
...
// static
bool EthereumRemoteCilentService::OpenSeed(const std::string& cipher_seed,
...
// static
std::string EthereumRemoteCilentService::GetRandomNonce() {
...
// static
std::string EthereumRemoteCilentService::GetRandomSeed() {
  // crypto::RandBytes is fail safe.
...
// static
bool EthereumRemoteCilentService::SealSeed(const std::string& seed,
                                  const std::string& key,
                                  const std::string& nonce,
                                  std::string* cipher_seed) {
  crypto::Aead aes_256_gcm_siv(crypto::Aead::AES_256_GCM_SIV);
...
// static
void EthereumRemoteCilentService::SaveToPrefs(PrefService* prefs,
...
void EthereumRemoteCilentService::ResetCryptoWallets() {
#if BUILDFLAG(ENABLE_EXTENSIONS)
...
bool EthereumRemoteCilentService::LoadRootSeedInfo(std::vector<uint8_t> key,
                                          std::string* seed) {
  std::string nonce;
  std::string cipher_seed;
  if (!seed) {
  ...
  if (EthereumRemoteCilentService::LoadFromPrefs(prefs, &cipher_seed, &nonce)) {
    // Decrypt the existing seed.
    if (!EthereumRemoteCilentService::OpenSeed(cipher_seed, aes_256_gcm_siv_key, nonce,
                                      seed)) {
      return false;
    }
  } else {
    // No valid previous value was stored, so generate new random values.
    nonce = EthereumRemoteCilentService::GetRandomNonce();
    *seed = EthereumRemoteCilentService::GetRandomSeed();
    // Encrypt that seed.
    if (!EthereumRemoteCilentService::SealSeed(*seed, aes_256_gcm_siv_key, nonce,
                                      &cipher_seed)) {
      return false;
    }
    // Save it to prefs.
    EthereumRemoteCilentService::SaveToPrefs(prefs, cipher_seed, nonce);
  }
  ...
  DCHECK_EQ(nonce.size(), EthereumRemoteCilentService::kNonceByteLength);
  DCHECK_EQ(seed->size(), EthereumRemoteCilentService::kSeedByteLength);
  return true;
}
...
// https://github.com/brave/brave-browser/wiki/Brave-Ethereum-Remote-Client-Wallet-Seed-Information
std::string EthereumRemoteCilentService::GetWalletSeed(std::vector<uint8_t> key) {
  std::string seed;
  if (!LoadRootSeedInfo(key, &seed)) {
    return "";
  }
  return EthereumRemoteCilentService::GetEthereumRemoteClientSeedFromRootSeed(seed);
}
...
// For more information, see:
// https://github.com/brave/brave-browser/wiki/Brave-Ethereum-Remote-Client-Wallet-Seed-Information
std::string EthereumRemoteCilentService::GetBitGoSeed(std::vector<uint8_t> key) {
  std::string seed;
  if (!LoadRootSeedInfo(key, &seed)) {
    return "";
  }
  return EthereumRemoteCilentService::GetBitGoSeedFromRootSeed(seed);
}

void EthereumRemoteCilentService::RemoveUnusedWeb3ProviderContentScripts() {
...
void EthereumRemoteCilentService::OnPreferenceChanged() {
  RemoveUnusedWeb3ProviderContentScripts();
}

#if BUILDFLAG(ENABLE_EXTENSIONS)
void EthereumRemoteCilentService::OnExtensionInstalled(
...
void EthereumRemoteCilentService::OnExtensionLoaded(
    content::BrowserContext* browser_context,
    const extensions::Extension* extension) {
...
void EthereumRemoteCilentService::OnExtensionUnloaded(
    content::BrowserContext* browser_context,
    const extensions::Extension* extension,
...
void EthereumRemoteCilentService::OnExtensionUninstalled(
    content::BrowserContext* browser_context,
    const extensions::Extension* extension,
...
void EthereumRemoteCilentService::CryptoWalletsExtensionReady() {
...
bool EthereumRemoteCilentService::IsCryptoWalletsSetup() const {
  PrefService* prefs = user_prefs::UserPrefs::Get(context_);
  return prefs->HasPrefPath(kBraveWalletAES256GCMSivNonce) &&
         prefs->HasPrefPath(kBraveWalletEncryptedSeed);
}

bool EthereumRemoteCilentService::IsCryptoWalletsReady() const {
#if BUILDFLAG(ENABLE_EXTENSIONS)
  auto* registry = extensions::ExtensionRegistry::Get(context_);
...
bool EthereumRemoteCilentService::ShouldShowLazyLoadInfobar() const {
  PrefService* prefs = user_prefs::UserPrefs::Get(context_);
  auto provider = static_cast<BraveWalletWeb3ProviderTypes>(
      prefs->GetInteger(kBraveWalletWeb3Provider));
...
void EthereumRemoteCilentService::MaybeLoadCryptoWalletsExtension(
