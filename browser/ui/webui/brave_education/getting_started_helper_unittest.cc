...
class GettingStartedHelperTest : public testing::Test {
 protected:
  static constexpr auto kDefaultContentType =
      EducationContentType::kGettingStartedAdBlock;

  void AddSuccessResponse(EducationContentType content_type) {
    auto url = GetEducationContentServerURL(content_type);
    test_url_loader_factory_.AddResponse(url.spec(), "success");
  }

  void AddErrorResponse(EducationContentType content_type) {
    auto url = GetEducationContentServerURL(content_type);
...
TEST_F(GettingStartedHelperTest, WithDefaultContentType) {
  ...
  AddSuccessResponse(kDefaultContentType);
  ...
  ASSERT_EQ(result.value(), GetEducationContentBrowserURL(kDefaultContentType));
}

TEST_F(GettingStartedHelperTest, WithPrivacyContentType) {
  base::test::ScopedFeatureList feature_list;
  feature_list.InitAndEnableFeatureWithParameters(
      features::kShowGettingStartedPage,
      {{"BraveGettingStartedContentType", "privacy"}});

  EducationContentType content_type =
      EducationContentType::kGettingStartedPrivacy;
  AddSuccessResponse(content_type);
  ...
  auto result = future.Get<0>();
  ASSERT_TRUE(result.has_value());
  ASSERT_EQ(result.value(), GetEducationContentBrowserURL(content_type));
}
...
TEST_F(GettingStartedHelperTest, BadResponse) {
  ...
  AddErrorResponse(kDefaultContentType);
