...
IN_PROC_BROWSER_TEST_F(BraveEducationUIBrowserTest, OpenShieldsSettings) {
  NavigateToEducationPage(EducationContentType::kGettingStartedAdBlock);

  content::WebContentsAddedObserver added_observer;

  PostMessageFromIFrame(R"(
      {messageType: 'browser-command',
       command: 'open-shields-settings'})");

  auto* new_web_contents = added_observer.GetWebContents();
  EXPECT_EQ(new_web_contents->GetVisibleURL(),
            GURL(chrome::GetSettingsUrl("shields")));
}

IN_PROC_BROWSER_TEST_F(BraveEducationUIBrowserTest, OpenPrivacySettings) {
  NavigateToEducationPage(EducationContentType::kGettingStartedPrivacy);
  ...
  auto* new_web_contents = added_observer.GetWebContents();
  EXPECT_EQ(new_web_contents->GetVisibleURL(),
            GURL(chrome::GetSettingsUrl("privacy")));
}

IN_PROC_BROWSER_TEST_F(BraveEducationUIBrowserTest, OpenWeb3Settings) {
  NavigateToEducationPage(EducationContentType::kGettingStartedWeb3);

  content::WebContentsAddedObserver added_observer;

  PostMessageFromIFrame(R"(
      {messageType: 'browser-command',
       command: 'open-web3-settings'})");

  auto* new_web_contents = added_observer.GetWebContents();
  EXPECT_EQ(new_web_contents->GetVisibleURL(),
            GURL(chrome::GetSettingsUrl("web3")));
}

IN_PROC_BROWSER_TEST_F(BraveEducationUIBrowserTest, OpenContentFilterSettings) {
  NavigateToEducationPage(EducationContentType::kGettingStartedAdBlock);
  ...
  PostMessageFromIFrame(R"(
      {messageType: 'browser-command',
       command: 'open-content-filter-settings'})");

  auto* new_web_contents = added_observer.GetWebContents();
  EXPECT_EQ(new_web_contents->GetVisibleURL(),
            GURL(chrome::GetSettingsUrl("shields/filters")));
}
