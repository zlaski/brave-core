...
class PrintPreviewExtractor : public printing::mojom::PrintPreviewUI {
 public:
  ...
  ~PrintPreviewExtractor() override { ClearPreviewUIId(); }
 ...
 private:
  void DeleteSelfAndSendResult(std::string result) {
  ...
  void DidPrepareDocumentForPreview(int32_t document_cookie,
    ...
    client->PrepareToCompositeDocument(
        ...
        mojo::WrapCallbackWithDefaultInvokeIfNotRun(
            base::BindOnce(
                &PrintPreviewExtractor::OnPrepareForDocumentToPdfDone,
  ...
  void OnGetOCRResult(std::string text) {
    DeleteSelfAndSendResult(std::move(text));
  }

  raw_ptr<content::WebContents> web_contents_ = nullptr;
  raw_ptr<Profile> profile_ = nullptr;
  bool is_pdf_ = false;
  AIChatTabHelper::ExtractPrintPreviewCallback callback_;
...
void ExtractPrintPreviewContent(
    ...
    AIChatTabHelper::ExtractPrintPreviewCallback callback) {
  // Self-deleting
  PrintPreviewExtractor* extractor = new PrintPreviewExtractor(
      web_contents,
      Profile::FromBrowserContext(web_contents->GetBrowserContext()), is_pdf,
      std::move(callback));
  extractor->CreatePrintPreview();
}
