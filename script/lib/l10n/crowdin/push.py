...
from lib.l10n.crowdin.common import (crowdin_name_from_filename,
                                     get_acceptable_json_lang_codes,
                                     get_crowdin_client_wrapper,
                                     get_strings_dict_from_xml_content,
                                     json_lang_to_crowdin_lang,
...
def upload_source_file_to_crowdin(channel, source_file_path, filename):
    ...
    assert ext == '.grd' or ext == '.json', ('Unsupported source file ext ' + 
...
def upload_json_translations_to_crowdin(channel, source_string_path,
    ...
    for lang_code in lang_codes:
        ...
        if not get_crowdin_client_wrapper().is_supported_language(crowdin_lang):
        ...
        for (string_name, string_value, _) in source_strings:
            if string_name not in l10n_dict:
                ...
                continue
            if len(l10n_dict[string_name]) == 0 or \
            ...
            translation_value = (l10n_dict[string_name]
                                 ...
                                 .replace("\r", "\\r")
                                 .replace("\n", "\\n"))
            translation_value = braveify(translation_value)
...
def upload_grd_translations_to_crowdin(channel, source_string_path, filename,
    ...
    for (lang, path) in grd_xtbs:
        ...
        for xtb_string in xtb_strings:
            ...
            if len(matches):
                key = matches[0][0]
                value = braveify(textify(xtb_string))
                if len(value) == 0:
...
def generate_source_strings_xml_from_grd(output_xml_file_path, grd_file_path):
    ...
    assert len(all_strings), f'GRD {grd_file_path} appears to be empty'
    for (string_name, string_value, _, string_desc) in all_strings:
        (string_value, string_desc) = process_source_string_value(
            string_value, string_desc)
        # Revert escaping of & because lxml.etree.tostring will do it again
        # and we'll end up with &amp;amp;
        resources_tag.append(create_android_format_string_tag(
...
def process_source_string_value(string_value, string_desc):
    ...
    for ph in phs:
        ...
        if ph.text is not None:
            ph.text = ''
    string_desc = add_placeholders_examples_to_description(string_desc,
                                                           examples)
