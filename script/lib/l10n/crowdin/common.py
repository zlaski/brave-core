...
brave_project_id = 6  # Core Test (Android+Chrome)
...
def crowdin_name_from_filename(source_file_path, filename):
    ...
    if 'brave-site-specific-scripts/scripts/' in source_file_path:
        return crowdin_name_from_greaselion_script_name(source_file_path)
...
def crowdin_name_from_greaselion_script_name(script_name):
    match = re.search(('brave-site-specific-scripts/scripts/(.*)/_locales/' +
                       'en_US/messages.json$'), script_name)
    if match:
        return ('greaselion_' +
            match.group(1).replace('-', '_').replace('/', '_') + '.json')
    return ''
...
def get_strings_dict_from_xml_content(xml_content):
    ...
    return {string_tag.get('name'): textify_from_crowdin(string_tag)
