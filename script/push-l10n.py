...
from lib.l10n.crowdin.push import (
    upload_grd_translations_to_crowdin,
...
def parse_args():
    ...
    parser.add_argument('--service', nargs=1,
                        choices=['Transifex', 'Crowdin'], default='Transifex')
...
def main():
    ...
    should_use_service_for_file = (
        (use_crowdin and should_use_crowdin_for_file(
            source_string_path, filename))
        or should_use_transifex_for_file(source_string_path, filename))
    ...
    if not should_use_service_for_file:
        ...
        upload_source_file(use_crowdin, channel, override_string_path,
                           filename)
        upload_source_strings_descriptions(
            use_crowdin, channel, override_string_path, filename)
    ...
    if ext == '.grd':
        ...
        check_source_grd_strings_parity_with_service(
            use_crowdin, channel, source_string_path)
    upload_source_strings_descriptions(
        use_crowdin, channel, source_string_path, filename)
    ...
    if ('brave-site-specific-scripts' in source_string_path or
        ...
        if ext == '.grd':
            ...
            upload_json_translations(use_crowdin, channel, source_string_path,
                missing_only = args.with_missing_translations)
...
def upload_source_strings_descriptions(
    use_crowdin, channel, source_string_path, filename):
...
def upload_grd_translations(use_crowdin, channel, source_string_path, filename,
    ...
    if use_crowdin:
        ...
        upload_grd_translations_to_transifex(
            source_string_path, filename, missing_only, is_override)
