...
class ScriptTracker {
 public:
  ...
  void AddScriptSourceForElm(const String& code,
                             const blink::DOMNodeId node_id);
  ...
  void AddCodeFetchedFromUrl(const String& code, const blink::KURL& url);
  void AddExtensionCodeFetchedFromUrl(const String& code,
                                      const UrlHash url_hash);

  // Method used for step 3 above.
  void SetScriptIdForCode(const ScriptId script_id, const String& code);
