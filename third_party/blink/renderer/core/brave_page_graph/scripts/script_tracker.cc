...
void ScriptTracker::AddScriptSourceForElm(const String& code,
                                          const DOMNodeId node_id) {
...
void ScriptTracker::AddCodeFetchedFromUrl(const String& code, const KURL& url) {
  const SourceCodeHash code_hash(code.Impl()->GetHash());
  const UrlHash url_hash(url.GetString().Impl()->GetHash());
...
void ScriptTracker::AddExtensionCodeFetchedFromUrl(const String& code,
