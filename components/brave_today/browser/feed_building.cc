...
bool ShouldDisplayFeedItem(const mojom::FeedItemPtr& feed_item,
  ...
  auto* data = MetadataFromFeedItem(feed_item)->get();
  ...
  auto* publisher_ptr = &(publishers->at(data->publisher_id));
  auto* publisher = publisher_ptr->get();
...
bool BuildFeed(const std::string& json,
  ...
  for (auto& item : feed_items) {
    ...
    auto* metadata = MetadataFromFeedItem(item)->get();
