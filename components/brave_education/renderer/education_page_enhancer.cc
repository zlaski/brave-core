...
constexpr std::array kAllowedHosts{"brave.com"};

constexpr std::array kAllowedPaths{"/whats-new"};

constexpr char16_t kEnhancePageScript[] =
    uR"js(
  ...
  actionHandlers.set('show-settings-page', (action) => {
    ...
    braveEducation.showSettingsPage(section, target)
  })
...
bool ShouldEnhancePage(content::RenderFrame* render_frame) {
  ...
  if (!origin.is_valid() || !origin.SchemeIs(url::kHttpsScheme) ||
      !base::Contains(kAllowedHosts, origin.host())) {
    return false;
  }
  ...
  for (const char* allowed_path : kAllowedPaths) {
...
std::optional<mojom::SettingsPageSection> ParseSettingsPageSection(
    const std::string& name) {
  if (name == "appearance") {
    return mojom::SettingsPageSection::kAppearance;
  }
  if (name == "shields/filters") {
    return mojom::SettingsPageSection::kShieldsFilters;
  }
  return std::nullopt;
}
...
void EducationPageEnhancer::InjectEducationRequestAPI(
  ...
  JSAPIBuilder::Create(blink::MainThreadIsolate(), context)
      .SetMethod("showSettingsPage",
                 base::BindRepeating(&EducationPageEnhancer::HandleShowSettings,
                                     weak_factory_.GetWeakPtr()))
      .SetAsObjectProperty(context->Global(), "braveEducation");
}

void EducationPageEnhancer::HandleShowSettings(const std::string& section,
                                               const std::string& target) {
  auto settings_page_section = ParseSettingsPageSection(section);
  if (!settings_page_section) {
    return;
  }

  GetRequestHandler()->ShowSettingsPage(*settings_page_section,
