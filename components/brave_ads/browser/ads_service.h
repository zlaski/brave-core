...
class AdsService : public KeyedService {
 public:
  ...
  // Called to get saved ads history between |from_time| and |to_time| date
  // range. The callback takes one argument - |base::Value::List| containing
  // info of the obtained history.
  virtual void GetSavedHistory(base::Time from_time,
                               base::Time to_time,
                               GetHistoryCallback callback) = 0;
