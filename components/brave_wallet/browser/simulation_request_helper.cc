...
std::optional<bool> HasEmptyRecentBlockhash(
    const mojom::SolanaTransactionRequestUnion& request) {
  if (request.is_sign_sol_transactions_request()) {
    const auto& sign_sol_transactions_request =
        request.get_sign_sol_transactions_request();
...
std::optional<std::pair<std::string, std::string>> EncodeScanTransactionParams(
  ...
  if (request.is_sign_sol_transactions_request()) {
    ...
    for (auto& tx_data : sign_sol_transactions_request->tx_datas) {
      auto serialized_tx =
          GetBase64TransactionFromSolanaTxData(std::move(tx_data));
      if (!serialized_tx) {
        return std::nullopt;
      }
