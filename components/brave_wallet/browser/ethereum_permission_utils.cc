...
bool AddAccountToHost(const url::Origin& old_origin,
  ...
  if (old_origin.host().empty() || account.empty() || !new_origin)
    return false;
...
void ExtractAddresses(const url::Origin& origin,
  ...
  DCHECK(!origin.host().empty() && address_queue);
...
bool GetConcatOriginFromWalletAddresses(
    ...
    url::Origin* new_origin) {
  if (old_origin.host().empty() || addresses.empty()) {
    return false;
  }
...
GURL GetConnectWithSiteWebUIURL(const GURL& webui_base_url,
  ...
  DCHECK(webui_base_url.is_valid() && !accounts.empty() &&
         !origin.host().empty());
