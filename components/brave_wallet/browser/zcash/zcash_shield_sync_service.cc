...
void ZCashShieldSyncService::OnGetTreeStateForChainReorg(
  ...
  if (!tree_state.has_value() || !tree_state.value() ||
    ...
    sync_state_.AsyncCall(&ZCashOrchardSyncState::HandleChainReorg)
...
void ZCashShieldSyncService::UpdateSubtreeRoots() {
  sync_state_.AsyncCall(&ZCashOrchardSyncState::GetLatestShardIndex)
      .WithArgs(account_id_.Clone())
...
void ZCashShieldSyncService::OnGetLatestShardIndex(
  ...
  if (!result.has_value()) {
    error_ = Error{ErrorCode::kDatabaseError, ""};
    ScheduleWorkOnTask();
    return;
  }
...
void ZCashShieldSyncService::OnGetSubtreeRoots(
  ...
  sync_state_.AsyncCall(&ZCashOrchardSyncState::UpdateSubtreeRoots)
      .WithArgs(account_id_.Clone(), std::move(result.value()))
