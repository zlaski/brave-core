...
function RegionCity(props: RegionCityProps) {
  return (
    <S.RegionCity>
      <S.RegionCityInfo>
        <S.RegionCityLabel>{props.cityLabel}</S.RegionCityLabel>
        <S.CityServerInfo>{props.serverInfo}</S.CityServerInfo>
      </S.RegionCityInfo>
      {props.selected && (
  ...
  return (
    ...
    >
      ...
      >
        ...
        <S.CountryInfo>
          <S.RegionCountryLabel>{props.region.namePretty}</S.RegionCountryLabel>
          <S.CountryServerInfo selected={props.selected}>
...
function RegionAutomatic() {
  ...
  const currentRegion = useSelector((state) => state.currentRegion)
  const handleConnect = () => {
    // Give empty region to clear selected region.
    // Then, service will pick default region.
    let empty_region = new Region()
    empty_region.name = ''
    empty_region.namePretty = ''
    empty_region.continent = ''
    empty_region.countryIsoCode = ''
    empty_region.regionPrecision = ''
    empty_region.cities = []
    empty_region.isAutomatic = true
    dispatch(Actions.connectToNewRegion(empty_region))
  }

  return (
    <S.RegionContainer selected={currentRegion.isAutomatic}>
      <S.RegionCountry>
        <Flag countryCode={'WORLDWIDE'} />
        <S.CountryInfo>
          <S.RegionCountryLabel>Automatic</S.RegionCountryLabel>
