...
export default class CosmeticBlockingControl extends React.PureComponent<Props, State> {
  ...
  get maybeBlock1stPartyAdsBlocked (): boolean {
    const { cosmeticBlocking } = this.props
    return maybeBlockResource(cosmeticBlocking)
  }

  onChange1stPartyAdsBlockedEnabled = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    return (
      <>
        <BlockedInfoRow id='cosmeticBlockerControl'>
            <BlockedInfoRowSingleText>{getLocale('firstPartyAdsBlocked')}</BlockedInfoRowSingleText>
            <Toggle
              id='cosmeticFiltering'
              size='small'
              disabled={isBlockedListOpen}
              checked={this.maybeBlock1stPartyAdsBlocked}
              onChange={this.onChange1stPartyAdsBlockedEnabled}
            />
        </BlockedInfoRow>
      </>
    )
